            ...
            foreach(var ret in retainerData)
            {
                ...
                }
                {
                    if(C.EntrustPlans.TryGetFirst(s => s.Guid == adata.EntrustPlan, out var plan))
                    {
                        ...
                        ImGui.PopFont();
                        ImGuiEx.Tooltip($"委托计划\"{plan.Name}\"已激活。" + (plan.ManualPlan ? "\n这是手动处理计划" : "") + (Utils.GetReachableRetainerBell(false) != null ? "\n点击进行委托。" : ""));
                ...
                ImGuiEx.Text($"{(!ret.HasVenture ? "无探险" : Utils.ToTimeString(ret.GetVentureSecondsRemaining(C.TimerAllowNegative)))}");
                ImGui.TableNextColumn();
                ImGui.TableSetBgColor(ImGuiTableBgTarget.CellBg, 0);
